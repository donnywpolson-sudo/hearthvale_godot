...
Expected output is 24 PNGs per viewport, `screenshot_manifest.json`, and `visual_review_prompt.md` at the timestamp root. The manifest records scenario categories, expected UI composition, enabled/disabled control checks, fixture timing, and blank/occlusion results, including sparse-sample and column-wide dark-region checks; the audit rejects stale, incomplete, duplicated, or contract-failing packets. Use the generated prompt for concrete visual defects only: overlap, clipping, missing assets, low contrast, blank panels, confusing selected/disabled states, missing interaction controls, bad z-order, cropped controls, or viewport framing problems. Do not use screenshot review as proof of fun, balance, audio quality, or player comprehension.
...
The current worktree includes a `Windows Desktop` x86_64 preset in
`export_presets.cfg`, but that file is currently untracked. The bounded export
command is:
...
Existing export notes conflict: an earlier local validation attempt reported
that matching Godot 4.7 Windows export templates were not installed, while a
later handoff entry reported successful executable export. This task did not
rerun export or inspect generated binaries, so current template state and
executable/platform parity are unverified here. The content-only command below
can validate project packing without Windows executable templates:
...
This validates the PCK only; it does not prove the Windows executable launches.

After installing the templates, use
`.\_ai_audit_workflow\_internal\run_export_platform_smoke.ps1` for the
repeatable export, short launch, and process-cleanup check. Its evidence is
written to `.godot_logs\export_platform_smoke.json`.
...
For the simplest workflow, run `_ai_audit_workflow\RUN_AUDIT.ps1`. It performs one bounded Focused audit and queue build by default, using full content probes for broader discovery, then prints the whole-audit pursue-goal prompt and exits without a one-item menu; use `-Tier Light` for the quick 120-run sample. Codex execution requires explicit `-NextFix` or `-Apply -MaxPasses N`. Use `-PlanOnly` for audit-only review, `-MaxPasses 2` or `-MaxPasses 3` for an explicitly bounded batch, and `-Manual` for the review menu. Supporting launchers, reports, and canonical config live under `_ai_audit_workflow\_internal`.
...
- Focused: estimated ~30-60 min, 600 runs, 600 steps, scenario/profile `all/coverage`, seed 1, trace `issues`, full scenario probes, content campaign, 90-minute stop budget. Use for deeper skill, recipe, resource, and mob coverage without an overnight run.
...
Tier runtimes are estimates. The named Light, Focused, and Deep tiers include nonzero stop budgets so the workflow cannot run uncapped; direct custom commands can still pass `--timeout-seconds 0` when an uncapped custom run is intentional.
...
```powershell
.\_ai_audit_workflow\_internal\HEARTHVALE_AI_SIMULATION.bat 120 200 1 all issues coverage 600 auto baseline
```

The optional seventh `.bat` argument is timeout seconds. `0` disables the timeout; positive values restore a runtime cap for a specific run. The optional eighth argument is scenario probes: `auto`, `off`, `smoke`, or `full`. The optional ninth argument is campaign: `baseline`, `adversarial`, `content`, `opportunity`, or `replay`.
...
The improvement queue has distinct lanes. `evidence-backed code fix` items stay first priority and require concrete current-run evidence plus replay/hash metadata before edits. `gameplay-opportunity` items are ranked, review-only hypotheses with a smallest experiment and acceptance criterion; they can never invoke Codex or authorize gameplay/content/balance/data changes. `review-backed polish fix` items are bounded visual/performance review prompts and must confirm a defect from screenshots, logs, telemetry, code, data, manual notes, smokes, reports, or findings before editing. `workflow-evidence-improvement` items are audit-harness or evidence-coverage work only; missing screenshot review, missing audio/manual/export evidence, telemetry false positives, and report-quality gaps must be reported as evidence gaps or workflow items, not gameplay defects. A failed audit invalidates the queue for apply/preview purposes until a fresh audit ends in `pass` or `pass with gaps`.
...
This catalog includes five current-worktree validation inputs that are currently
untracked: `export_presets.cfg`,
`scripts/audio_feedback_smoke.gd`, `scripts/quest_route_smoke.gd`,
`scripts/skill_enrichment_smoke.gd`, and
`scripts/tools/ai_audit_workflow_contract_smoke.gd`. Until their delivery status
is decided, this catalog is not a clean-checkout reproducibility claim.

The configured focused matrix contains 28 smoke scripts; the table below lists
all 25 expected checks.
...
| V2 migration, corruption recovery, account identity, and atomic promotion | `res://scripts/persistence_v2_smoke.gd` | `Hearthvale persistence v2 smoke passed.` |
| Actual scene wiring, realtime timers, autosave debounce, and exit flush | `res://scripts/runtime_integration_smoke.gd` | `Hearthvale runtime integration smoke passed.` |
...
| Collision-safe multi-drop loot, player drops, pickup isolation, and reload | `res://scripts/ground_loot_regression_smoke.gd` | `Hearthvale ground loot regression smoke passed.` |
...
| Audio asset loading and intentional silent action/UI feedback | `res://scripts/audio_feedback_smoke.gd` | `Hearthvale audio feedback smoke passed.` |
...
| Woodcutting/Carpentry, Mining/Smithing, Fishing/Cooking, and Strength/Ranged/Magic mastery enrichment | `res://scripts/skill_enrichment_smoke.gd` | `Hearthvale skill enrichment smoke passed.` |
| Bank, shop, NPC dialogue, quests, rewards | `res://scripts/economy_quest_smoke.gd` | `Hearthvale economy and quest smoke passed.` |
| Deliberate quest route selection, reward capacity, and quest save/load continuity | `res://scripts/quest_route_smoke.gd` | `Hearthvale quest route smoke passed.` |
...
| AI audit workflow contract and queue policy | `res://scripts/tools/ai_audit_workflow_contract_smoke.gd` | `Hearthvale AI workflow contract smoke passed.` |
...
Historical Step 9 result: all focused checks passed with
`C:\Users\donny\Desktop\Godot_v4.7-stable_win64.exe`. This result predates the
current dirty worktree and is not proof of current validation; current results
must be tied to a dated run against the current files.
...
Gameplay, World, and HUD smoke scenarios live under `scripts/test_support/` as inherited harnesses. The documented `res://scripts/*_smoke.gd` entry points and pass messages remain stable; production classes contain runtime behavior only.

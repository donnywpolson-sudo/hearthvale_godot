...
function Get-CheckDisplayDetail {
    param(
        [Parameter(Mandatory)][string] $Name,
        [Parameter(Mandatory)][string] $Fallback
    )
    $detail = [string](Get-CheckDetail -Name $Name)
...
function Write-AuditOutputs {
    ...
    $lines.Add('')
    $lines.Add('## Blocking Result')
    $lines.Add('')
    if ($Status -eq 'fail') {
        $blocker = if ($Failure.Trim().Length -gt 0) { $Failure } else { 'A required audit workflow check failed.' }
        $lines.Add('- Result: `FAIL`.')
        $lines.Add("- Blocking check: $blocker")
        $lines.Add('- Do not use this run to apply queued gameplay, polish, or workflow evidence items.')
        $lines.Add('- Next valid action: fix the blocking workflow/check failure, then run a fresh audit.')
    } elseif ($Status -eq 'pass with gaps') {
        $lines.Add('- Result: `PASS WITH GAPS`.')
        $lines.Add('- Required automation completed, but residual evidence gaps remain manual or review-only.')
        $lines.Add('- Queued items must stay inside their evidence lane.')
    } else {
        $lines.Add('- Result: `PASS`.')
        $lines.Add('- Required automation completed without blocking workflow failures.')
    }
    $lines.Add('')
    ...
    $lines.Add('| --- | --- | --- |')
    $visualCaptureDetail = Get-CheckDisplayDetail -Name 'visual_capture' -Fallback $visualFolderText
    $simulationDetail = Get-CheckDisplayDetail -Name 'simulation' -Fallback "$($config.simulationSummary); publish/latest status $publishStatus"
    $focusedSmokeDetail = Get-CheckDisplayDetail -Name 'focused_smoke_matrix' -Fallback "$($smokeRows.Count)/$($config.smokes.Count) smoke result row(s) recorded"
    $diffHygieneDetail = Get-CheckDisplayDetail -Name 'diff_hygiene' -Fallback 'git diff --check'
    $lines.Add("| Visible screenshot capture | $(Get-CheckStatus -Name 'visual_capture') | $visualCaptureDetail |")
    $lines.Add("| Strategy simulation | $(Get-CheckStatus -Name 'simulation') | $simulationDetail |")
    $lines.Add("| Focused smoke matrix | $(Get-CheckStatus -Name 'focused_smoke_matrix') | $focusedSmokeDetail |")
    $lines.Add("| Diff hygiene | $(Get-CheckStatus -Name 'diff_hygiene') | $diffHygieneDetail |")
    $lines.Add('')
    ...
    $directSimulationClassification = if ((Get-CheckStatus -Name 'simulation') -eq 'passed') { 'proven' } else { 'not proven' }
    $focusedSmokesClassification = if ((Get-CheckStatus -Name 'focused_smoke_matrix') -eq 'passed') { 'proven' } else { 'not proven' }
    $visualScreenshotsClassification = if ($null -ne $VisualFolder) { 'proven' } else { 'not proven' }
    $lines.Add("| Direct simulation | $directSimulationClassification | $($config.simulationSummary) | Generated findings still require replay/hash review before code changes. |")
    ...
    $lines.Add("| Focused smokes | $focusedSmokesClassification | $($smokeRows.Count)/$($config.smokes.Count) smoke result row(s). | Does not prove long-run balance or subjective quality. |")
    $lines.Add("| Visual screenshots | $visualScreenshotsClassification | $visualFolderText | Requires human/AI review for concrete visual defects. |")
...
try {
    ...
    if ($SkipSmokes) {
        ...
        foreach ($smoke in $config.smokes) {
            ...
            $expectedMessage = [string](Get-ObjectProperty -Object $smoke -Name 'expected' -Default '')
            $passedMessage = $false
            for ($attempt = 0; $attempt -lt 50; $attempt++) {
                ...
                if (([string]$logText).Contains($expectedMessage)) {
                    $passedMessage = $true
                    break
                }
            ...
            if (-not $passedMessage) {
                throw "Smoke $($smoke.name) exited 0 but expected pass message was not found: $expectedMessage"
            }
    ...
    if ($_.InvocationInfo -and $_.InvocationInfo.PositionMessage) {
        $failureMessage = "$failureMessage $($_.InvocationInfo.PositionMessage)"
    }
    $failureMessage = ($failureMessage -replace '\s+', ' ').Trim()
    if ($failureMessage -like 'git diff --check failed*' -and (Get-CheckStatus -Name 'diff_hygiene' -Default '') -eq '') {
        Add-Check -Name 'diff_hygiene' -Status 'failed' -Detail $failureMessage
    }

...
func _run() -> void:
	...
	watchdog.timeout.connect(func() -> void:
		push_error("Hearthvale save/load torture smoke timed out.")
		quit(1)
	)
	...
	store.free()
	if passed:
		print("Hearthvale save/load torture smoke passed.")
		quit(0)
	else:
		push_error("Hearthvale save/load torture smoke failed.")
		quit(1)

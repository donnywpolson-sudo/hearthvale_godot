...
func run_playable_shell_smoke() -> bool:
	...
	var zoom_start := camera.position.z
	_zoom_in()
	if camera.position.z >= zoom_start:
		return false
	_zoom_out()
	_zoom_out()
	if camera.position.z <= zoom_start:
...
func _set_destination_tile(tile: Vector2i) -> void:
...
func _apply_zoom(amount: float) -> void:
	camera.position.z = clamp(camera.position.z + amount, MIN_CAMERA_DISTANCE, MAX_CAMERA_DISTANCE)
